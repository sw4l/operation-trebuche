...
*/
private ["_launchPos","_target"];

_launchPos1 = if ( typeName (_this select 0) == "Object") then { getPos (_this select 0) } else { (_this select 0) };
_launchPos = [_launchPos1, 50] call CBA_fnc_randPos;
_target = if ( typeName (_this select 1) == "Object") then { getPos (_this select 1) } else { (_this select 1) };
_side = _this select 2; 
_logic = _this select 3;
_messageText = _this select 4;

[ [((switch (_logic getVariable "Faction") do { case 0: { WEST }; case 1: { EAST }; case 2: { Independent }; case 3: { Civilian };})), _messageText, "mp_groundsupport_70_tacticalstrikeinbound_BHQ_0"] , "BWCSM_fnc_GlobalSideChat" , true , false, false ] call BIS_fnc_MP; 

if (_logic getVariable "ClassName" != "") then { playSound3D ["A3\Sounds_F\sfx\alarm_independent.wss", _logic]; };
sleep 6;
...
playSound3D ["BWCSM_Vehicles\CruiseMissile\Data\Sound\cruisemissile_launch.wss", _logic, false, [_x, _y , 1], 1, 1, 2000]; 
...
_wp setWaypointStatements ["true", "((group this) getVariable ""BWCSM_CruiseMissileTarget"") spawn BWCSM_fnc_CruiseMissileDecent"];
